...
class OSWorldACI: ACI {
    ...
    func executeActionCode(_ code: String) throws {
        ...
        if code.hasPrefix("CLICK(") {
            // Parse CLICK command and execute
            // Simplified for now - would need full parser
            try executeClickCode(code)
        } else if code.hasPrefix("TYPE(") {
            try executeTypeCode(code)
        } else if code == "DONE" {
